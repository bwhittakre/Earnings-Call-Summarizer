...
2. For each entry, call Quartr `read_transcript` for `event_id` and write
   `host_quartr/transcripts/{event_id}.json`.
3. Run `python -m services.earnings_monitor.host_automation live --max-workers 4 --final`.
4. Stop.
...
| `skipped_reason=missing_dump` | MCP dump not written for that event_id |
...
3. `host_automation calendar` → manifests + worklist.
4. Drop `host_quartr/transcripts/{event_id}.json` fixtures.
5. `host_automation live --max-workers 2 --final` → inbox bundles.
6. `watchlist remove --ticker OPAL --period FY2026-Q2 --prune-manifests` →
