...
# -*- coding: utf-8 -*-
"""
Focus 3b novelty-scoring orchestrator for narrative-only dimensions.
...
from novelty_scorer import NoveltyScorer  # noqa: E402
...
def main() -> int:
    ...
    print(f"Scoring narrative novelty for {len(output_quarters)} quarter(s) "
    ...
    for q in output_quarters:
        ...
        prior_q = q_by_fp.get(prior_fp) if prior_fp else None
        if prior_q is None:
        ...
        print(f"[{fp}] scoring narrative novelty…")
        scored = scorer.score(transcript, prior_block, company.company_name)
        ...
        (audit_dir / f"{fp}_novelty.json").write_text(
    ...
    if needs_merge:
        ...
        merged_df = merge_dataframes_by_period(existing_df, pd.DataFrame(rows), scored_periods)
        rows = merged_df.to_dict("records")
        if existing_view:
            quarters_view = merge_quarter_views(
                existing_view.get("quarters", []),
                quarters_view,
                scored_periods,
            )
    ...
    novelty_view_file.write_text(json.dumps(novelty_view, indent=2), encoding="utf-8")
